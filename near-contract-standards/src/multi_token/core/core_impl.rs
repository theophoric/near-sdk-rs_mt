...
use crate::multi_token::core::resolver::{MultiTokenResolver, ext_mt_resolver};
...
use near_sdk::{
    assert_one_yocto, env, log, require, AccountId, Balance, BorshStorageKey,
...
impl MultiTokenCore for MultiToken {
    ...
    fn mt_transfer_call(
        ...
        ext_mt_receiver::ext(receiver_id.clone())
            .with_static_gas(env::prepaid_gas() - GAS_FOR_MT_TRANSFER_CALL)
            .mt_on_transfer(sender_id, vec![old_owner.clone()], vec![token_id.clone()], vec![amount], msg)
            .then(
    ...
    fn mt_batch_transfer_call(
        ...
        ext_mt_receiver::ext(receiver_id.clone())
            ...
            .then(
                ext_mt_resolver::ext(env::current_account_id())
                    .with_static_gas(GAS_FOR_RESOLVE_TRANSFER)
                    .mt_resolve_transfer(old_owners, receiver_id, token_ids, amounts, Some(old_approvals))
            )
        .into()
