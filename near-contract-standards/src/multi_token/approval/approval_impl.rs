use crate::multi_token::approval::{MultiTokenApproval, ext_mt_approval_receiver};
...
use near_sdk::{assert_one_yocto, env, Gas, require, AccountId, Balance, Promise};
use std::collections::HashMap;

const GAS_FOR_MT_APPROVE: Gas = Gas(10_000_000_000_000);
...
impl MultiTokenApproval for MultiToken {
    fn mt_approve(
        ...
        msg.map(|msg| {
            ext_mt_approval_receiver::ext(grantee_id)
                .with_static_gas(env::prepaid_gas() - GAS_FOR_MT_APPROVE )
                .mt_on_approve(token_ids,approver_id, new_approval_ids, msg)
        })
    }
